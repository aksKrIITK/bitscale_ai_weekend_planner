...
async def test_city_planning():
    print("\n==========================================")
    print("Testing Mumbai Itinerary with Groq LLM")
    print("==========================================")
    mumbai_request = PlanRequest(
        city="Mumbai",
        budget=2500,
        available_time="4 hours",
        mood="relaxed and cultural",
        interests=["art", "walks", "coffee"],
        constraints=["vegetarian", "avoid crowded places"]
    )
    res = await planner_service.create_plan(mumbai_request, session_id="test-mumbai-session")
...
if __name__ == "__main__":
    asyncio.run(test_city_planning())
